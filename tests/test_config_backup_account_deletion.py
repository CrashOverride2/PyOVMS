"""
Deleting an account deletes its configuration backups — permanently, on every path.

A snapshot is someone's themes, vehicle names and custom commands, and nothing else
in the account can reproduce it; once the account is gone, the rows must be too.
There are four ways an account goes: an administrator over the API, an administrator
on the users page, the user on their own profile, and the lifecycle housekeeping. All
four end in `crud.user.delete_user()`, and the rows leave through the ORM cascade on
`User.config_backups` — which matters, because SQLite does not enforce the column's
`ON DELETE CASCADE` at all (see the PRAGMA note in app/database.py). Every path is
driven here in full, over HTTP where there is HTTP, with another user's rows alongside
to prove the deletion stops at the account boundary.
"""
...
def _fill(db, user):
    """Both kinds, two devices — every bucket the retention rules know."""
    for kind, device, n in (("auto", "aaaa", 1), ("auto", "aaaa", 2),
                            ("manual", "aaaa", 3), ("auto", "bbbb", 4), ("manual", None, 5)):
        crud_backup.store_backup(
            db, user.id, kind=kind, payload=json.dumps({"schemaVersion": 1, "settings": {"n": n}}),
            device_id=device, device_name="Phone", label=None, app_version=None,
            platform=None, schema_version=1,
        )


def _backups_of(db, user_id) -> int:
    return db.query(models_db.ConfigBackup).filter_by(owner_id=user_id).count()
...
def doomed_and_bystander(db):
    """Two accounts with backups, as ids — the doomed instance itself is unusable
    once its row is gone. `bystander` must come out untouched."""
    doomed = _make_user(db, "doomed")
    bystander = _make_user(db, "bystander")
    _fill(db, doomed)
    _fill(db, bystander)
    assert _backups_of(db, doomed.id) == 4, "auto rows coalesce; the fixture holds what it says"
    assert _backups_of(db, bystander.id) == 4
...
def _assert_only_the_bystander_remains(db, doomed_id, bystander_id):
    ...
    assert _backups_of(db, doomed_id) == 0, "the account's backups outlived it"
    assert _backups_of(db, bystander_id) == 4, "another user's backups were touched"


# --- the four paths -------------------------------------------------------------------

def test_an_administrator_deleting_over_the_api_takes_the_backups(client, db, doomed_and_bystander):
...
def test_an_administrator_deleting_on_the_users_page_takes_the_backups(client, db, doomed_and_bystander):
    doomed_id, bystander_id = doomed_and_bystander
...
def test_a_user_deleting_their_own_account_takes_the_backups(client, db, doomed_and_bystander):
...
def test_the_lifecycle_housekeeping_path_takes_the_backups(db, doomed_and_bystander):
...
def test_the_constraint_cascades_where_the_database_enforces_it():
    ...
    with a backup impossible to delete on those backends."""
    ...
    try:
        ...
        session.commit()
        _fill(session, user)
        assert _backups_of(session, user.id) == 4
        ...
        assert session.query(models_db.ConfigBackup).count() == 0
    finally:
        session.close()
        fk_engine.dispose()

    fk = next(fk for fk in models_db.ConfigBackup.__table__.foreign_keys if fk.column.table.name == "users")
