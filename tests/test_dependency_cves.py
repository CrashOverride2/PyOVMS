...
Two different kinds of check live here, and they are deliberately not the same kind:

  * The lockfile checks are properties of two files in the repository. They are offline
    and deterministic, so they run in the ordinary suite like everything else.

  * The CVE scan needs the current advisory database, which means network. The rest of
    this suite is offline on purpose (see conftest.py), and a test that silently turns
    green when it cannot reach PyPI would be worse than no test — so it is marked `cve`
    and deselected by default. Run it explicitly:
...
#                  so a package without a wheel for the running interpreter fails the
#                  audit for a reason that has nothing to do with security. Karto's
#                  numpy pin does exactly that on Python 3.14. Together they make this a
#                  pure offline read of the pinned file.
_PIP_AUDIT_ARGS = ["--no-deps", "--disable-pip", "--strict", "--progress-spinner=off"]


def _normalise(name: str) -> str:
    """PEP 503 normalisation: Jinja2, jinja2 and jinja-2 are one package."""
...
def test_every_declared_dependency_is_pinned():
    ...
    and forgets to re-run the compile. The server then works on their machine, where
...
def test_ci_workflow_uses_the_same_flags():
    ...
    the runner's Python. So the flags are compared instead. Dropping --no-deps in one
    place only would leave two audits that disagree about which packages they cover,
    and the disagreement would show up as an advisory nobody sees.
    """
...
def test_python_jose_stays_out():
    """
    It was removed for pulling in ecdsa (CVE-2024-23342, Minerva timing attack, no
    upstream fix) while never being imported — JWTs use pyjwt. requirements.in says so
    in a comment, and a comment does not survive a hurried merge.
    """
    pins = _pinned_packages()
    for banned in ("python-jose", "ecdsa"):
        assert banned not in pins, (
            f"{banned} is back in requirements.txt. See the NOTE at the bottom of "
            f"requirements.in for why it was removed."
        )
...
def test_no_known_vulnerabilities_in_pinned_dependencies():
    ...
    tree against the *running* interpreter, and this repository compiles its lockfile on
    one Python version and deploys on another — the audit would then describe a set that
    nobody installs.
    """
...
def test_pip_audit_is_available_to_ci():
    ...
    exact false green this file exists to avoid, so when the marker is asked for
    explicitly, the tool has to be there.
    """
