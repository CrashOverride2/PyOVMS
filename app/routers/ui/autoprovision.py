...
def ui_add_autoprovision_profile_submit_route(
    ...
    return RedirectResponse(url=f"{request.url_for('ui_manage_autoprovision_profiles')}?success_message=Profile '{ap_key}' created successfully.", status_code=status.HTTP_303_SEE_OTHER)
...
def ui_edit_autoprovision_profile_form_route(
    ...
    if not profile_to_edit:
        return RedirectResponse(url=f"{request.url_for('ui_manage_autoprovision_profiles')}?error_message=Profile not found.", status_code=status.HTTP_303_SEE_OTHER)
...
def ui_edit_autoprovision_profile_submit_route(
    ...
    return RedirectResponse(url=f"{request.url_for('ui_manage_autoprovision_profiles')}?success_message=Profile '{ap_key}' updated successfully.", status_code=status.HTTP_303_SEE_OTHER)
...
def ui_delete_autoprovision_profile_route(
    ...
    if not profile_to_delete:
        return RedirectResponse(url=f"{redirect_url}?error_message=Profile not found.", status_code=status.HTTP_303_SEE_OTHER)
    
    deleted_key = profile_to_delete.ap_key
    crud.autoprovision.delete_auto_provision_profile(db, profile_id)
        
    return RedirectResponse(url=f"{redirect_url}?success_message=Profile '{deleted_key}' deleted successfully.", status_code=status.HTTP_303_SEE_OTHER)