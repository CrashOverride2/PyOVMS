...
def ui_add_user_submit_route(
    ...
    try:
        ...
        return RedirectResponse(url=f"{redirect_url_on_error}?error_message={quote_plus(str(e.detail))}", status_code=status.HTTP_303_SEE_OTHER)

    if timezone not in available_timezones():
        return RedirectResponse(url=f"{redirect_url_on_error}?error_message=Invalid timezone selected.", status_code=status.HTTP_303_SEE_OTHER)
    ...
    try:
        ...
        return RedirectResponse(url=f"{redirect_url_on_error}?error_message={quote_plus(str(_('Disposable email addresses are not allowed.')))}", status_code=status.HTTP_303_SEE_OTHER)
        ...
        return RedirectResponse(url=f"{redirect_url_on_error}?error_message=Unable to validate email domain right now. Please try again later.", status_code=status.HTTP_303_SEE_OTHER)
    ...
    try:
        ...
        pass
    return RedirectResponse(url=f"{request.url_for('ui_manage_users')}?success_message=User '{username}' created successfully.", status_code=status.HTTP_303_SEE_OTHER)
...
def ui_edit_user_form_route(
    ...
    if not user_to_edit:
        return RedirectResponse(url=f"{request.url_for('ui_manage_users')}?error_message=User not found.", status_code=status.HTTP_303_SEE_OTHER)
...
def ui_edit_user_submit_route(
    ...
    if user_db.id == current_admin.id:
        if not is_active: return RedirectResponse(url=f"{redirect_url_on_error}?error_message=Admin cannot deactivate themselves.", status_code=status.HTTP_303_SEE_OTHER)
        if not is_admin: return RedirectResponse(url=f"{redirect_url_on_error}?error_message=Admin cannot remove their own admin status.", status_code=status.HTTP_303_SEE_OTHER)
    ...
    if is_active != old_is_active:
        try:
            ...
            pass
    return RedirectResponse(url=f"{request.url_for('ui_manage_users')}?success_message=User '{username}' updated successfully.", status_code=status.HTTP_303_SEE_OTHER)
...
async def ui_delete_user_route(
    ...
    try:
        ...
        return RedirectResponse(url=f"{redirect_url}?error_message={quote_plus(str(e.detail))}", status_code=status.HTTP_303_SEE_OTHER)

    user_to_delete = crud.user.get_user_by_id(db, user_id)

    if not user_to_delete:
        return RedirectResponse(url=f"{redirect_url}?error_message=User not found.", status_code=status.HTTP_303_SEE_OTHER)

    if user_to_delete.id == current_admin.id:
        return RedirectResponse(url=f"{redirect_url}?error_message=Admin cannot delete themselves.", status_code=status.HTTP_303_SEE_OTHER)
    ...
    try:
        ...
        return RedirectResponse(
            url=f"{redirect_url}?error_message="
                f"{quote_plus('Trip data could not be deleted right now, so the account was kept. Please retry.')}",
    ...
    if not crud.user.delete_user(db, user_id):
        return RedirectResponse(url=f"{redirect_url}?error_message={quote_plus(f'Could not delete user {deleted_username}. They may own vehicles or AP profiles.')}", status_code=status.HTTP_303_SEE_OTHER)
    ...
    try:
        ...
        pass
    return RedirectResponse(url=f"{redirect_url}?success_message=User '{deleted_username}' deleted successfully.", status_code=status.HTTP_303_SEE_OTHER)
...
def ui_clear_user_auto_delete_route(
    ...
    return RedirectResponse(url=f"{redirect_url}?success_message={quote_plus(f'Auto-delete cancelled for user {user.username}.')}", status_code=status.HTTP_303_SEE_OTHER)
